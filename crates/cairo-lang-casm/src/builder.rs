...
use crate::instructions::{
    AddApInstruction, AssertEqInstruction, Blake2bCompressInstruction, Blake2sCompressInstruction,
    CallInstruction, Instruction, InstructionBody, JnzInstruction, JumpInstruction, RetInstruction,
};
...
impl CasmBuilder {
    ...
    /// Add a statement performing Blake2b compression.
    ///
    /// `state` must be a cell reference to a pointer to `[u64; 8]` as the hash state.
    /// `byte_count` must be a cell reference to the number of bytes in the message.
    /// `message` must be a cell reference to a pointer to `[u64; 16]` as the message.
    /// `finalize` should be `true` if this is the final compression.
    ///
    /// Additionally the pointer to the output hash state should be on the top of the stack.
    pub fn blake2b_compress(&mut self, state: Var, byte_count: Var, message: Var, finalize: bool) {
        let instruction = self.next_instruction(
            InstructionBody::Blake2bCompress(Blake2bCompressInstruction {
                state: self.as_cell_ref(state, true),
                byte_count: self.as_cell_ref(byte_count, true),
                message: self.as_cell_ref(message, true),
                finalize,
